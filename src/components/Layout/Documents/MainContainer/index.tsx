  ...
  const DATA = [
    {
      ...
      video: '1.gif',
      description: {
        kor: `“Infinitas(∞)”는 옷을 형상화하는 골조로 ‘선’에 접근한 스테이지이다. 평면에 그려지는 선과 선이 교차하며 형성된 공간은 개인의 개성을 드러내는 이미지로 형상화된다. 원단 위에 그려지는 무한한 선의 상상을 통해 새로운 이미지를 사유하면서 사유의 접점에서 패션의 가능성을 발견하고자 하였다.`,
        eng: `"Infinitas(∞)" is a stage that approaches the 'line' with a frame that embodies clothes. The space formed by crossing the line drawn on the plane is shaped as an image that reveals an individual's personality. Through the imagination of infinite lines drawn on the fabric, we tried to discover the possibility of fashion at the interface of thought while thinking about a new image.`,
      },
    },
    {
      id: 'Spectrum',
    ...
    },
    {
      ...
      video: '4.gif',
      description: {
        kor: `“Textureacity”는 소재의 가능성을 보여줌과 동시에 다양한 존재들의 낯설지만 아름다운 조화를 탐구한 스테이지이다. 소재를 바탕으로 8명의 디자이너가 도달하고자 하는 지향점은 다르지만, 서로의 차이를 인정하고 이를 표현하는 과정에서 모든 디자이너들의 개성은 하나의 새로운 아름다움으로 수렴되었다. 각자 다른 소재의 결합으로부터 오는 조화와 대비는 패션의 실루엣부터 분위기까지 변화시키면서 의상에서의 무한한 잠재력을 보여주고 있다.`,
        eng: `“Textureacity” is a stage that shows the potential of fabrics and explores the unfamiliar but beautiful harmony of various beings. The eight designers' goals are different based on each fabric. In the process of acknowledging and expressing their differences, however, all designers' personalities converged into one new beauty. Harmony and contrast from the combination of different fabrics change from the silhouette to the mood of style, showing the infinite potential in fashion.`,
      },
    },
  ];
    ...
    <STDContainer>
      <STDContentContainer>
        {DATA.map(({ id, video, description }) => (
          ...
          >
            <STDGradientBox />
            <STDGifImage src={`image/documents/${video}`} />
